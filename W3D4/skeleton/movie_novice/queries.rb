...
def star_wars
  ...
  # hint: use 'select' and 'where'
  Movie.select("movies.id, movies.title, movies.yr").where("movies.title like 'Star Wars%' ")
end
...
def below_average_years
  ...
  Movie.select("movies.yr, count(*) as bad_movies").group("yr").where("score < 5").order("yr desc")
end
...
def pulp_fiction_actors
  ...
  Movie.select("actors.id, actors.name").joins(:actors).where("movies.title = 'Pulp Fiction'")
end
...
def uma_movies
  ...
  Actor.select("movies.id, movies.title, movies.yr").joins(:movies).where("actors.name = 'Uma Thurman'").order("movies.yr asc")
end
